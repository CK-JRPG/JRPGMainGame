...
void UCombatSynergyPointSubsystem::OnWorldBeginPlay(UWorld &InWorld)
{
...
void UCombatSynergyPointSubsystem::TryBindExternalEvents()
{
	...
	{
		Battle->OnBattleEnded.AddUObject(this, &UCombatSynergyPointSubsystem::HandleBattleEnded);
	}

	if (UChainAttackSubsystem* Chain = GetWorld()->GetSubsystem<UChainAttackSubsystem>())
	{
		Chain->OnChainAttackEnded.AddUObject(this, &UCombatSynergyPointSubsystem::HandleChainEnded);
	}

	bSubscribed = true;
}

bool UCombatSynergyPointSubsystem::CanAcceptGain() const
{
	if (!GetWorld())
		return false;

	const UCombatBattleSessionSubsystem* Battle = GetWorld()->GetSubsystem<UCombatBattleSessionSubsystem>();
	return Battle && Battle->IsBattleActive();
}

EPartyRole UCombatSynergyPointSubsystem::ResolveRoleForActor(AActor* Actor) const
{
	if (!Actor)
...
bool UCombatSynergyPointSubsystem::IsAlly(AActor* A,AActor* B) const
{
	if (!A||!B)
		return false;
...
bool UCombatSynergyPointSubsystem::IsEnemy(AActor* A,AActor* B) const
{
	if (!A || !B)
		return false;
...
int32 UCombatSynergyPointSubsystem::ComputeTacticalBonus(int32 RoleBonus,bool bFromTacticalReservation) const
{
...
bool UCombatSynergyPointSubsystem::PassesSameEventCooldown(const FString &EventKey,double Now)
{
	if (EventKey.IsEmpty())
	{
		return true;
...
int32 UCombatSynergyPointSubsystem::ConsumePerSecondBudget(int32 ProposedAmount,double Now)
{
	if (ProposedAmount <= 0)
		return 0;
...
void UCombatSynergyPointSubsystem::UpdateReadyState()
{
...
void UCombatSynergyPointSubsystem::ApplyGainEvent(FSPGainEvent &Event)
{
	if (!CanAcceptGain())
		return;
...
void UCombatSynergyPointSubsystem::ResetForBattleEnd(FName ReasonTag)
{
...
void UCombatSynergyPointSubsystem::ResetForChainEnd(FName ReasonTag)
{
	const int32 Prev =State.CurrentSP;
...
void UCombatSynergyPointSubsystem::HandleBattleEnded(const FBattleSessionSnapshot &, EBattleEndReason)
{
	ResetForBattleEnd("SP.Reset.BattleEnd");
}

void UCombatSynergyPointSubsystem::HandleChainEnded(const FChainAttackSnapshot &)
{
	ResetForChainEnd("SP.Reset.ChainEnd");
}

void UCombatSynergyPointSubsystem::ReportDamage(AActor *Instigator, AActor *Target,
...
void UCombatSynergyPointSubsystem::ReportHeal(AActor *Instigator, AActor *Target,
...
void UCombatSynergyPointSubsystem::ReportBreak(AActor*Instigator, AActor*Target,
...
void UCombatSynergyPointSubsystem::ReportBuff(AActor *Instigator, AActor *Target, FName BuffId,
...
void UCombatSynergyPointSubsystem::ReportDebuff(AActor *Instigator, AActor *Target, FName DebuffId,
	bool bFromTacticalReservation, FName ReasonTag)
{
...
void UCombatSynergyPointSubsystem::ReportCleanse(AActor *Instigator, AActor *Target,
...
void UCombatSynergyPointSubsystem::ReportThreatOutcome(AActor* Instigator, AActor* EnemyOwner,
...
void UCombatSynergyPointSubsystem::ReportAggroHold(AActor* Defender, AActor* EnemyOwner,
	bool bFromTacticalReservation,FName ReasonTag)
{
...
void UCombatSynergyPointSubsystem::ReportPartyProtect(AActor* Defender,AActor* ProtectedTarget,
	bool bFromTacticalReservation,FName ReasonTag)
{
