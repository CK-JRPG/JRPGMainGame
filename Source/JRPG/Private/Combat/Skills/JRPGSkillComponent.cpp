#include "JRPG/Public/Combat/Skills/JRPGSkillComponent.h"
...
void UJRPGSkillComponent::BeginPlay()
{
	...
	for (UJRPGSkillDataAsset *S : KnownSkills)
	{
...
void UJRPGSkillComponent::TickComponent(float DeltaTime, ELevelTick, FActorComponentTickFunction*)
{
...
UJRPGSkillDataAsset* UJRPGSkillComponent::GetSkillDef(FName SkillId) const
{
	for (UJRPGSkillDataAsset *S : KnownSkills)
	{
...
void UJRPGSkillComponent::LearnSkill(UJRPGSkillDataAsset *Skill)
{
...
bool UJRPGSkillComponent::IsHostileTarget(AActor *Target)const
{
	if (!GetOwner() || !Target)
		return false;
...
FSkillCastResult UJRPGSkillComponent::ValidateCast(const UJRPGSkillDataAsset &Skill,const TArray<AActor*> &Targets) const
{
...
FSkillCastResult UJRPGSkillComponent::PrepareSkillCast(FName SkillId, const TArray<AActor*>&Targets, bool bFromTacticalReservation, FName ReasonTag)
{
	UJRPGSkillDataAsset* Skill = GetSkillDef(SkillId);
	if (!Skill)
...
void UJRPGSkillComponent::ApplySkillEffects(const UJRPGSkillDataAsset &Skill, const TArray<AActor*>&Targets, bool bFromTacticalReservation)
{
	...
	for (AActor *T :Targets)
	{
		...
		UCombatHPComponent* THP = T->FindComponentByClass<UCombatHPComponent>();
		UCombatStatsComponent* TStats = T->FindComponentByClass<UCombatStatsComponent>();
		UCombatThreatComponent* TThreat = T->FindComponentByClass<UCombatThreatComponent>();
...
FSkillCastResult UJRPGSkillComponent::ResolvePreparedSkillCast()
{
...
void UJRPGSkillComponent::CancelPreparedSkillCast(bool bRefundCost, FName)
{
...
bool UJRPGSkillComponent::HasPreparedSkillCast()const
{
	return bHasPrepared && Prepared.Skill != nullptr;
}

FSkillCastResult UJRPGSkillComponent::CastSkill(FName SkillId,const TArray<AActor*> &Targets,FName ReasonTag)
{
