...
ACharacter* UJRPGCombatMotionComponent::GetCharacterOwner() const
{
	return Cast<ACharacter>(GetOwner());
}

UCombatBattleSessionSubsystem* UJRPGCombatMotionComponent::GetBattle() const
{
	return GetWorld() ? GetWorld()->GetSubsystem<UCombatBattleSessionSubsystem>() : nullptr;
}

int32 UJRPGCombatMotionComponent::GetPriorityForType(ECombatMotionType Type) const
{
	switch (Type)
	{
...
bool UJRPGCombatMotionComponent::ValidateRequest(const FCombatMotionRequest& Req, FName& OutReason) const
{
	OutReason = NAME_None;
...
FVector UJRPGCombatMotionComponent::MakeDirectionFromTarget(AActor* TargetActor) const
{
...
bool UJRPGCombatMotionComponent::ResolveRequestContext(FCombatMotionRequest& InOutReq, FName& OutReason)
{
	OutReason = NAME_None;
...
bool UJRPGCombatMotionComponent::CanReplaceCurrent(const FCombatMotionRequest& NewReq, FName& OutReason) const
{
	OutReason = NAME_None;
...
void UJRPGCombatMotionComponent::StartAcceptedMotion(const FCombatMotionRequest& Req, const FCombatMotionHandle& Handle, bool bBroadcastReplaced, const FCombatMotionHandle& ReplacedHandle)
{
...
void UJRPGCombatMotionComponent::FinishTeleportRequestImmediately(const FCombatMotionRequest& Req, const FCombatMotionHandle& Handle)
{
	bool bClamped = false;
...
FCombatMotionResponse UJRPGCombatMotionComponent::RequestCombatMotion(const FCombatMotionRequest& InReq)
{
...
bool UJRPGCombatMotionComponent::CancelCombatMotion(FCombatMotionHandle Handle, FName ReasonTag)
{
...
int32 UJRPGCombatMotionComponent::CancelAllByOwner(FName OwnerTag, FName ReasonTag)
{
...
void UJRPGCombatMotionComponent::EndActiveMotion(FName EndReasonTag)
{
	if (UCombatDebugSubsystem* Debug = GetWorld() ? GetWorld()->GetSubsystem<UCombatDebugSubsystem>() : nullptr)
	{
		Debug->AddLog(
...
FVector UJRPGCombatMotionComponent::ClampLocationToBattle(const FVector& InLocation, bool& bWasClamped) const
{
	bWasClamped = false;
...
void UJRPGCombatMotionComponent::ApplyClampIfNeeded()
{
	if (!IsMotionActive()) return;
...
void UJRPGCombatMotionComponent::TickVelocityCurve(float DeltaTime)
{
...
void UJRPGCombatMotionComponent::TickRootMotion(float DeltaTime)
{
	MotionState.Elapsed += DeltaTime;
	ApplyClampIfNeeded();
	if (!IsMotionActive()) return;
...
void UJRPGCombatMotionComponent::TickComponent(float DeltaTime, ELevelTick, FActorComponentTickFunction*)
{
	if (!IsMotionActive()) return;
